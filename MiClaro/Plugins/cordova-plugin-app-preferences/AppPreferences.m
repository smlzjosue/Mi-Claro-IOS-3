...
- (void)store:(CDVInvokedUrlCommand*)command
{
	...
	//id dataStore = [self getStoreForOptions:options];
    id dataStore = [[NSUserDefaults alloc] initWithSuiteName:settingsDict];
	...
	@try {
        ...
        sharedDefaults = [[NSUserDefaults alloc] initWithSuiteName:settingsDict];

        if ([settingsType isEqual: @"string"] && [JSONObj isKindOfClass:[NSString class]]) {
            [sharedDefaults setObject:settingsValue forKey:settingsName];
		} else if ([settingsType  isEqual: @"number"] && [JSONObj isKindOfClass:[NSNumber class]]) {
			[sharedDefaults setObject:(NSNumber*)JSONObj forKey:settingsName];
			// setInteger: forKey, setFloat: forKey:
		} else if ([settingsType  isEqual: @"boolean"]) {
			[sharedDefaults setObject:JSONObj forKey:settingsName];
		} else {
			// data
			[sharedDefaults setObject:settingsValue forKey:settingsName];
		}
